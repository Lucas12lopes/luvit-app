...
faqs.forEach(([question, answer], i) => { const item = document.createElement("div"); item.className = "faq-item"; const button = document.createElement("button"); button.type = "button"; button.className = "faq-question"; button.id = `faq-button-${i}`; button.setAttribute("aria-expanded", "false"); button.setAttribute("aria-controls", `faq-answer-${i}`); const text = document.createElement("span"); text.textContent = question; const mark = document.createElement("span"); mark.textContent = "+"; const panel = document.createElement("div"); panel.className = "faq-answer"; panel.id = `faq-answer-${i}`; panel.setAttribute("role", "region"); panel.setAttribute("aria-labelledby", button.id); panel.textContent = answer; button.append(text, mark); button.addEventListener("click", () => { const open = button.getAttribute("aria-expanded") === "true"; button.setAttribute("aria-expanded", String(!open)); mark.textContent = open ? "+" : "−"; }); item.append(button, panel); faqList?.append(item); });
...
const demoContent = { plan: ["PLANEJAMENTO", "Organize as paradas", "Selecione uma parada para ver os detalhes"], optimize: ["SEQUÊNCIA SUGERIDA", "Compare antes de aplicar", "A otimização só altera a rota depois da sua confirmação"], deliver: ["MODO EM ROTA", "Acompanhe a jornada", "Parada atual, próxima entrega e progresso no mesmo lugar"] };
qsa("[data-demo]").forEach(tab => tab.addEventListener("click", () => { qsa("[data-demo]").forEach(item => { const active = item === tab; item.classList.toggle("active", active); item.setAttribute("aria-selected", String(active)); }); const [eyebrow, title, label] = demoContent[tab.dataset.demo]; qs("#demo-eyebrow").textContent = eyebrow; qs("#demo-title").textContent = title; qs("#demo-map-label").textContent = label; qsa(".demo-stop").forEach((stop, index) => stop.classList.toggle("active", index === ({ plan: 0, optimize: 1, deliver: 2 }[tab.dataset.demo]))); }));
